...
# #depededing on live method, embed these children templates into it, depending on user actions
# #fix the otree to 5.10.4
# #sample code base outdated

# #main change - use self as first argument, replaced all methods with static methods and global functions on other end

# #run command within settings.py

# #create subtemplate html file, include folder, put some html files - files will not be 
# #full templates but blocks of html code instead. general structure of the page will embed the 
# #children templates depending on the live methods. the result = using live methods can display different
# #contents while keeping participant content on the same page

# #no longer a page showing submission, but just update on backend and then update content


# #regarding pics, first include everything and then hide everything via css

# # from models, pages import into init but first do from otree.api import *, then run no self and constant commans, move each class outside each function, no self, update as global
# #  functions, just link to player not session and others
# # for judge if we want unique id, then dont use integers, use 